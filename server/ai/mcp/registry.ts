 ...
 * sees a mutating tool. A tool's `requiredCapabilities` (ANY-OF) and optional
 * `requiredAllCapabilities` gates must also pass. An MCP caller can never invoke a tool the granting capabilities
...
import { componentMcpTools } from './tools/componentTools'
...
// Each is shadowed by a headless version ordered earlier in the registry:
//   - `site_list_tokens` / `site_list_breakpoints` → `styleMcpTools`.
